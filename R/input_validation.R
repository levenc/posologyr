...
validate_priormod <- function(priormod){
  if (is.null(priormod$ppk_model)){
    stop("ppk_model is missing from the prior posologyr model")}
  if (is.null(priormod$error_model)){
    stop("error_model is missing from the prior posologyr model")}
  if (is.null(priormod$theta)){
    stop("theta is missing from the prior posologyr model")}
  if (is.null(priormod$omega)){
    stop("omega is missing from the prior posologyr model")}
  if (is.null(priormod$sigma)){
    stop("sigma is missing from the prior posologyr model")}
  ...
  if (FALSE %in% (attr(omega_eta,"dimnames")[[1]] %in%
                  priormod$ppk_model$params)){
    stop("The names of the omega matrix do not match the parameters of
         ppk_model")
  }
  if (FALSE %in% (attr(priormod$theta,"names") %in% priormod$ppk_model$params)){
    stop("The names of the theta vector do not match the parameters of
         ppk_model")
  }
}
  ...
  if (any(class(object$sigma) == "list",class(object$error_model) == "list")) {
    ...
    if (any(!is.null(names(object$sigma)),!is.null(names(object$error_model)))){
      if (!setequal(names(object$sigma),names(object$error_model))){
        stop("The names of the sigma matrices do not match the names of the
             error models")
      }
      if (FALSE %in% ((unique(object$tdm_data$DVID) %in% names(object$sigma)))){
        stop("The observation types in DVID do not match the names of the
             error models")
      }
    } else {
      stop("The sigma and error models lists must be named lists, with names
          matching the DVID column from the patient record")}
