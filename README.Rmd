...
`posologyr` is an R package for the individualization of drug dosage, taking advantage of population pharmacokinetics, patient characteristics, and the results of therapeutic drug monitoring.

`posologyr` provides functions for estimating the pharmacokinetic (PK) parameters:

+ `poso_simu_pop()` for estimating the a priori distributions of PK parameters by Monte-Carlo simulations
...
+ `poso_estim_sir()` for estimating the posterior distributions of individual PK parameters by Sequential Importance Resampling (SIR)
