...
prior_dose <- poso_dose_conc(patA_prior_amik,
                             time_c = 1,
                             duration = 0.5,
                             target_conc = 80)$dose
prior_dose
...
An event table is created to simulate the selected dosage with rxode2.

```{r prior_plot_et}
# infusion of prior_dose mg over 0.5 hours at time 0
event_table_priordose <- rxode2::et(time=0,amt=prior_dose,dur=0.5)
event_table_priordose$add.sampling(seq(0,7,by=0.1))
```
...
prior_dose_model_patA <- rxode2::rxSolve(object=patA_prior_amik$ppk_model,
...
map_dose <- poso_dose_conc(patA_tdm_amik,
                            time_c = 1,
                            duration = 0.5,
                            target_conc = 80)$dose
map_dose
...
map_dose_model_patA <- rxode2::rxSolve(object=patA_tdm_amik$ppk_model,
...
#Simulate with the event table and the individual MAP parameters
map_dose_model_patA_ss <- rxode2::rxSolve(object=patA_tdm_amik$ppk_model,
                                  params=cbind(map_param_patA,
                                            CLCREAT4H=50,
                                            TBW=62,
                                            PoverF=169),
